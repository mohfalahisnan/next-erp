...
export const POST = async (req: Request, {params}: {params: Promise<{model: string[]}>}) => {
	const {model} = await params;
	const modelName = model[0] as unknown as any;
	...
	try {
		...
		const res = createApiResponse(data, 1, 1, 1, "Created successfully", "CREATED");
...
export const PUT = async (req: Request, {params}: {params: Promise<{model: string[]}>}) => {
	const {model} = await params;
	const modelName = model[0] as unknown as any;
	const id = model[1];
	...
	if (!id) {
		return NextResponse.json(
			createApiError("ID is required for PUT request", "Bad Request", "BAD_REQUEST"),
			{ status: 400 }
		);
	}
	...
	try {
		...
		const res = createApiResponse(data, 1, 1, 1, "Updated successfully", "OK");
		return NextResponse.json(res);
	} catch (error: any) {
...
export const PATCH = async (req: Request, {params}: {params: Promise<{model: string[]}>}) => {
	const {model} = await params;
	const modelName = model[0] as unknown as any;
	const id = model[1];
	...
	if (!id) {
		return NextResponse.json(
			createApiError("ID is required for PATCH request", "Bad Request", "BAD_REQUEST"),
			{ status: 400 }
		);
	}
...
export const DELETE = async (_req: Request, {params}: {params: Promise<{model: string[]}>}) => {
	const {model} = await params;
	const modelName = model[0] as unknown as any;
	const id = model[1];
	...
	if (!id) {
		return NextResponse.json(
			createApiError("ID is required for DELETE request", "Bad Request", "BAD_REQUEST"),
			{ status: 400 }
		);
	}
	...
	try {
		...
		const res = createApiResponse([], 0, 1, 0, "Deleted successfully", "OK");
		return NextResponse.json(res);
	} catch (error: any) {
