...
async function testPOST() {
	...
	if (result.data?.data?.id) {
		console.log(
			'✅ POST test passed - User created with ID:',
			result.data.data.id
		);
...
async function testPUTWithPopulate(userId) {
	...
	const result = await makeRequest(
		'PUT',
		`${BASE_URL}/user/${userId}?populate=role`,
		updatedUser
	);
	console.log('Status:', result.status);
	console.log('Response:', JSON.stringify(result.data, null, 2));
...
async function testErrorHandling() {
	...
	const invalidPost = await makeRequest('POST', `${BASE_URL}/user`, {
		name: 'Only Name',
	});
	console.log('Invalid POST Status:', invalidPost.status);
	console.log(
		'Invalid POST Response:',
		JSON.stringify(invalidPost.data, null, 2)
	);

	// Test PUT without ID
	console.log('\n--- Testing PUT without ID ---');
	const putWithoutId = await makeRequest(
		'PUT',
		`${BASE_URL}/user`,
		updatedUser
	);
	...
	console.log(
		'PUT without ID Response:',
		JSON.stringify(putWithoutId.data, null, 2)
	);
	...
	console.log(
		...
		JSON.stringify(deleteNonExistent.data, null, 2)
	);
	...
	console.log(
		...
		JSON.stringify(invalidModel.data, null, 2)
	);
}
...
async function testDifferentModels() {
	...
	const createDept = await makeRequest(
		'POST',
		`${BASE_URL}/department`,
		deptData
	);
	...
	console.log(
		...
		JSON.stringify(createDept.data, null, 2)
	);
	...
	if (createDept.data?.data?.id) {
		...
		// Update department
		const updateDept = await makeRequest(
			'PUT',
			`${BASE_URL}/department/${deptId}`,
			{
		...
		// Delete department
		const deleteDept = await makeRequest(
			'DELETE',
			`${BASE_URL}/department/${deptId}`
		);
