...
async function testIdWithPopulate() {
	...
	try {
		...
		if (!usersResponse.data.data || usersResponse.data.data.length === 0) {
			console.log(
				'❌ No users found. Please ensure database has test data.'
			);
			return;
		}
		...
		const basicUserResponse = await axios.get(
		...
		const userWithRoleResponse = await axios.get(
			`${BASE_URL}/user/${testUserId}?populate=role`
		);
		console.log(`✅ Status: ${userWithRoleResponse.status}`);
		console.log(`✅ User: ${userWithRoleResponse.data.data.name}`);
		console.log(
			`✅ Role: ${userWithRoleResponse.data.data.role?.name || 'No role'}\n`
		);

		// Test 3: ID fetch with multiple populate
		console.log(
			'🔗 Test 3: ID fetch with multiple populate (role, department)'
		);
		const userWithMultipleResponse = await axios.get(
			`${BASE_URL}/user/${testUserId}?populate=role,department`
		);
		console.log(`✅ Status: ${userWithMultipleResponse.status}`);
		console.log(`✅ User: ${userWithMultipleResponse.data.data.name}`);
		console.log(
			`✅ Role: ${userWithMultipleResponse.data.data.role?.name || 'No role'}`
		);
		console.log(
			`✅ Department: ${userWithMultipleResponse.data.data.department?.name || 'No department'}\n`
		);

		// Test 4: ID fetch with depth
		console.log('🔗 Test 4: ID fetch with depth (role with depth=2)');
		const userWithDepthResponse = await axios.get(
			`${BASE_URL}/user/${testUserId}?populate=role&depth=2`
		);
		console.log(`✅ Status: ${userWithDepthResponse.status}`);
		console.log(`✅ User: ${userWithDepthResponse.data.data.name}`);
		console.log(
			`✅ Role data:`,
			JSON.stringify(userWithDepthResponse.data.data.role, null, 2)
		);
		console.log();

		// Test 5: ID fetch with all relations (depth only)
		console.log('🔗 Test 5: ID fetch with all relations (depth=2)');
		const userAllRelationsResponse = await axios.get(
			`${BASE_URL}/user/${testUserId}?depth=2`
		);
		console.log(`✅ Status: ${userAllRelationsResponse.status}`);
		console.log(`✅ User with all relations:`);
		console.log(
			JSON.stringify(userAllRelationsResponse.data.data, null, 2)
		);
		console.log();
		...
		const allUsersWithRoleResponse = await axios.get(
			`${BASE_URL}/user?populate=role`
		);
		const singleUserWithRoleResponse = await axios.get(
			`${BASE_URL}/user/${testUserId}?populate=role`
		);

		console.log(
			`✅ All users response type: ${Array.isArray(allUsersWithRoleResponse.data.data) ? 'Array' : 'Object'}`
		);
		console.log(
			`✅ Single user response type: ${Array.isArray(singleUserWithRoleResponse.data.data) ? 'Array' : 'Object'}`
		);
		console.log(
			`✅ All users count: ${allUsersWithRoleResponse.data.data.length}`
		);
		console.log(
			`✅ Single user ID matches: ${singleUserWithRoleResponse.data.data.id === testUserId}\n`
		);
		...
		if (
			...
			const deptWithUsersResponse = await axios.get(
				`${BASE_URL}/department/${testDeptId}?populate=users&depth=2`
			);
			console.log(`✅ Status: ${deptWithUsersResponse.status}`);
			console.log(
				`✅ Department: ${deptWithUsersResponse.data.data.name}`
			);
			console.log(
				`✅ Users in department: ${deptWithUsersResponse.data.data.users?.length || 0}`
			);
		} else {
...
async function performanceTest() {
	...
	try {
		...
		await axios.get(
			`${BASE_URL}/user/${testUserId}?populate=role,department&depth=2`
		);
		...
		console.log(
			`📈 Performance improvement: ${(((timeAll - timeSingle) / timeAll) * 100).toFixed(1)}%`
		);
	} catch (error) {
