...
const { parseModelRelations, generateModelRelationsString } = require('./scripts/generate-model-relations.js');
const { modelRelations, getModelRelations, isValidRelation } = require('./src/lib/populate-utils.ts');
...
console.log('📊 Total models in current mapping:', Object.keys(modelRelations).length);
console.log();
...
for (const testCase of testCases) {
  ...
  console.log(`${status} ${testCase.model}.${testCase.relation} -> ${result} (expected: ${testCase.expected})`);
}
console.log();
...
try {
  ...
  const missingInCurrent = freshModels.filter(model => !currentModels.includes(model));
  const extraInCurrent = currentModels.filter(model => !freshModels.includes(model));
  ...
  const commonModels = currentModels.filter(model => freshModels.includes(model));
  ...
  for (const model of commonModels) {
    ...
    const missingRels = freshRels.filter(rel => !currentRels.includes(rel));
  ...
  if (relationDifferences === 0) {
    ...
    console.log(`⚠️  Found ${relationDifferences} models with relation differences`);
  }
