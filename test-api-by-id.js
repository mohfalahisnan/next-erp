...
async function testApiById() {
	...
	try {
		...
		console.log(
			`Records found: ${allUsersResponse.data.data?.length || 0}`
		);
		...
		if (
			...
			const singleUserResponse = await axios.get(
				`${BASE_URL}/user/${firstUserId}`
			);
			console.log(`Status: ${singleUserResponse.status}`);
			console.log(
				`User found: ${singleUserResponse.data.data?.id || 'none'}`
			);
			console.log(
				`User name: ${singleUserResponse.data.data?.name || 'N/A'}\n`
			);

			// Test 3: Get user by ID with populate
			console.log(
				'🔗 Test 3: Get user by ID with populate (role, department)'
			);
			const populatedUserResponse = await axios.get(
				`${BASE_URL}/user/${firstUserId}?populate=role,department&depth=2`
			);
			console.log(`Status: ${populatedUserResponse.status}`);
			console.log(
				`User with relations:`,
				JSON.stringify(populatedUserResponse.data.data, null, 2)
			);
			console.log();
		}
		...
		console.log(
			`Departments found: ${departmentsResponse.data.data?.length || 0}`
		);
		...
		if (
			...
			const singleDeptResponse = await axios.get(
				`${BASE_URL}/department/${firstDeptId}?populate=users`
			);
			console.log(`Status: ${singleDeptResponse.status}`);
			console.log(
				`Department: ${singleDeptResponse.data.data?.name || 'N/A'}`
			);
			console.log(
				`Users in department: ${singleDeptResponse.data.data?.users?.length || 0}\n`
			);
		}
