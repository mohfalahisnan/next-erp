...
const POPULATE_UTILS_PATH = path.join(
	__dirname,
	'..',
	'src',
	'lib',
...
function updatePopulateUtils(newModelRelationsString) {
	...
	if (!startMatch) {
		console.error(
			'Could not find modelRelations export in populate-utils.ts'
		);
		return false;
	}
...
function main() {
	...
	try {
		...
		if (success) {
			console.log(
				'✅ Successfully updated modelRelations in populate-utils.ts'
			);
